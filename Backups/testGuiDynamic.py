...
def WebReg():
    ...
    buttonPlus = customtkinter.CTkButton(master=frame, text="+", command=lambda: [plus(newwin)])
    ...
    button1 = customtkinter.CTkButton(master=frame, text="Next", command=lambda: ([webLists.append(w.get()) for w in entry_widget], verify(newwin)))
